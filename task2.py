...
print("The f(x) derivative is", diff(cos(x)/(+x**2)))

def df(x):
    return -m.sin(x)/x**2-2*m.cos(x)/x**3
...
print("The executing time is", timeit.timeit('opt.newton(f,b/2-a/2,fprime=df,tol=1e-15)', setup='from __main__ import opt, m, f, df, a, b',  number=1000)/1000)
